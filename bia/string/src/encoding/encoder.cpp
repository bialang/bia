...
string encoder::convert(encoder& from, const std::int8_t* input, std::size_t length)
{
    ...
    auto buffer = static_cast<std::int8_t*>(gc::gc::active_gc()->allocate(to->max_size(from.max_code_points(length) + 1)));
...
#elif defined(BIA_STRING_ENCODING_BACKEND_NONE)
    switch (type) {
    case STANDARD_ENCODING::ASCII:
        return &ascii_encoder;
    case STANDARD_ENCODING::UTF_8:
        return &utf8_encoder;
    case STANDARD_ENCODING::UTF_16_LE:
        return &utf16le_encoder;
    case STANDARD_ENCODING::UTF_16_BE:
        return &utf16be_encoder;
    default:
        throw exception::unknown_encoder_exception(u"unsupported string encoding");
    }
#endif
}
