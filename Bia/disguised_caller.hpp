...
inline machine::memory::allocation<Class> disguised_caller(utility::type_container<Arguments...>, utility::type_container<>, framework::member::parameter_count_t _count, machine::stack * _stack, sequence<Indices...>)
{
	constexpr auto arg_count = sizeof...(Arguments);
...
template<typename Class, typename... Arguments, typename... Optionals, machine::stack::index_t... Indices>
inline machine::memory::allocation<Class> disguised_caller(utility::type_container<Arguments...>, utility::type_container<Optionals...>, framework::member::parameter_count_t _count, machine::stack * _stack, sequence<Indices...>)
{
	constexpr auto arg_count = sizeof...(Arguments);

	if (_count == arg_count) {
		return machine::machine_context::active_allocator()->construct<Class>(_stack->cast<framework::member*>(Indices - arg_count)->cast<Arguments>()..., Optionals()...);
	}

	using splitter = type_splitter<arg_count + 1, Arguments..., Optionals...>;

	return disguised_caller<Class>(splitter::first(), splitter::second(), _count, _stack, make_sequence<arg_count + 1>());
}

template<typename Class, typename... Arguments, machine::stack::index_t... Indices>
inline machine::memory::allocation<Class> disguised_caller(utility::type_container<Arguments...>, utility::type_container<>, const char * _format, framework::member::parameter_count_t _count, machine::stack * _stack, sequence<Indices...>)
{
	constexpr auto arg_count = sizeof...(Arguments);
...
template<typename Class, typename... Arguments, typename... Optionals, machine::stack::index_t... Indices>
inline machine::memory::allocation<Class> disguised_caller(utility::type_container<Arguments...>, utility::type_container<Optionals...>, const char * _format, framework::member::parameter_count_t _count, machine::stack * _stack, sequence<Indices...>)
{
	constexpr auto arg_count = sizeof...(Arguments);

	if (_count == arg_count) {
		return machine::machine_context::active_allocator()->construct<Class>(_stack->format_cast<Arguments>(Indices - arg_count, _format[Indices])..., Optionals()...);
	}

	using splitter = type_splitter<arg_count + 1, Arguments..., Optionals...>;

	return disguised_caller<Class>(splitter::first(), splitter::second(), _format, _count, _stack, make_sequence<arg_count + 1>());
}
...
inline machine::memory::allocation<Class> disguised_caller(Format _format, framework::member::parameter_count_t _count, machine::stack * _stack)
{
	...
	if (std::is_null_pointer<Format>::value) {
		return disguised_caller<Class>(splitter::first(), splitter::second(), _count, _stack, make_sequence<arg_count>());
	} else {
		return disguised_caller<Class>(splitter::first(), splitter::second(), static_cast<const char*>(_format), _count, _stack, make_sequence<arg_count>());
	}
}
