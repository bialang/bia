...
mpir_allocator::int_type * mpir_allocator::new_int()
{
	auto _int = static_cast<int_type*>(_allocator->allocate(sizeof(int_type) + sizeof(*int_type::_mp_d) * 2).first);
	
	// Set
	memset(_int, 0, sizeof(int_type));

	_int->_mp_alloc = 2;
	_int->_mp_d = reinterpret_cast<decltype(int_type::_mp_d)>(_int + 1);
