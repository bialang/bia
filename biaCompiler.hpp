...
class BiaCompiler final : grammar::BiaReportReceiver
{
public:
	/**
	 * Constructor.
	 ...
	 */
	inline BiaCompiler(stream::BiaOutputStream & p_output, machine::BiaMachineContext & p_context) : m_output(p_output), m_context(p_context)
	{
		_TOOLSET::Initialize(p_output, p_context);
	}
	inline BiaCompiler(BiaCompiler && p_move)
	{
...
private:
	stream::BiaOutputStream & m_output;	/**	Defines the destination of the machine code.	*/
	...
	inline const grammar::Report * HandleVariableDeclaration(grammar::report_range p_reports)
	{
		...
		_TOOLSET::PushParameter(m_output, reinterpret_cast<uintptr_t>(m_context.AddressOf(StringKey(p_reports.pBegin[1].token.pcString, p_reports.pBegin[1].token.iSize))));

		//Handle value and prepare the result for a function call
		HandleValue(pRight->content.children);

		//Make call
		_TOOLSET::Call(m_output, reinterpret_cast<const void*>(&machine::BiaMachineContext::InstantiateInt_32));
